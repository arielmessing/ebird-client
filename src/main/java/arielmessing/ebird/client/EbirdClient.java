...
public final class EbirdClient {

    private static final String DEFAULT_BASE_URL = "https://api.ebird.org/v2/";
    private static final String BASE_URL_PROPERTY = "client.base.url";
    ...
    public EbirdClient(HttpClient httpClient, ObjectMapper objectMapper) {
        this(System.getProperty(BASE_URL_PROPERTY, DEFAULT_BASE_URL), httpClient, objectMapper);
    }

    public EbirdClient(String baseUrl, HttpClient httpClient, ObjectMapper objectMapper) {
    ...
    public <T> T getResource(String resourcePath, String token, Class<T> responseType) {
        try {
            ...
            if (response.statusCode() == STATUS_OK) {
                ...
                throw new EbirdClientException("Request failed with status: " + response.statusCode());
            }
        } catch (JsonProcessingException e) {
            throw new EbirdClientException("Error parsing response", e);

        } catch (IOException | InterruptedException e) {
            throw new EbirdClientException("Error during API call", e);
        }
    }
}
