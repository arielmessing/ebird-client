...
import static arielmessing.ebird.client.EbirdClient.*;
...
class EbirdClientTest {
    ...
    private EbirdClient client;

    @BeforeEach
    void setUp() {
        client = new EbirdClient(mockHttpClient, mockObjectMapper);
    }
    ...
    void getResource_handleBadRequest() throws Exception {
        ...
        var error = new EbirdBadRequest.Error("status", "code", "title");
        when(mockObjectMapper.readValue(anyString(), eq(EbirdBadRequest.class)))
                .thenReturn(new EbirdBadRequest(new EbirdBadRequest.Error[] { error }));
    ...
    void getResource_handleServerError() throws Exception {
        ...
        EbirdClientException ex = assertThrows(
                EbirdClientException.class,
                () -> client.getResource("path/to/resource", "token", Object.class)
        );
